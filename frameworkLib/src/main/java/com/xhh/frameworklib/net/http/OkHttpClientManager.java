package com.xhh.frameworklib.net.http;
...
import com.xhh.frameworklib.common.utils.NetworkUtilBase;
import com.xhh.frameworklib.common.utils.TxtUtilsBase;
...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager() {
        cache = new Cache(new File(BaseConfig.SAVE_SD_FLODER), 10240 * 1024);
...
//        OkHttpUtils.initClient(mOkHttpClient);
    ...
    private void _getAsyn(String url, final ResultCallback callback) {
        final Request request = new Request.Builder()
            .url(url)
            .addHeader("TerminalCode", BaseConfig.TerminalCode)
            .addHeader("TerminalVersion", BaseConfig.TerminalVersion)
            .addHeader("Appid", BaseConfig.Appid)
            .addHeader("AppSecret", BaseConfig.AppSecret)
            .build();
        deliveryResult(callback, request);
    }

    private void _getAsyn(String url, final ResultCallback callback, String params) {
        Request request = null;
        if (TxtUtilsBase.isEmpty(params)) {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                try {
                    ...
                    if (callback.mType == String.class) {//如果下载文件成功，第一个参数为文件的绝对路径
                        sendSuccessResultCallback(request, file.getAbsolutePath(), callback, response.headers());
                    }
                } catch (IOException e) {
    ...
    //*************对外公布的方法************
    public static void getAsyn(boolean addSSL, String url, String params, ResultCallback callback) {
        getInstance(addSSL)._getAsyn(url, callback, params);
    }
    ...
    private Request buildMultipartFormRequestFile(String url, File[] file, String form) {
        MultipartBody.Builder builder = new MultipartBody.Builder()
            ...
            .addFormDataPart("allowSuffix", "jpg,png,bmp,jpeg,zip,rar,doc")
...
//            .addFormDataPart("moduleType", "0");
                ...
                if (response.code() == 200) {
                    try {
                        ...
                        if (callback.mType == String.class) {
                            sendSuccessResultCallback(request, string, callback, response.headers());
                        } else {
                            Object o = new Gson().fromJson(string, callback.mType);
                            sendSuccessResultCallback(request, o, callback, response.headers());
                        }
                    } catch (IOException e) {
                        sendFailedStringCallback(response.request(), e, response.code(), callback);
    ...
    private void sendSuccessResultCallback(final Request request, final Object object, final ResultCallback callback, final Headers headers) {
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
                if (callback != null) {
                    callback.onResponse(request, object, headers);
                }
            }
        });
    }
    ...
    private Request buildPostStringRequest(String url, String params) {
        ...
        return new Request.Builder()
            ...
            .addHeader("Appid", BaseConfig.Appid)
            .addHeader("AppSecret", BaseConfig.AppSecret)
            //                .addHeader("TerminalVersion", "v"+BaseConfig.clientVersion)
//                .addHeader("TerminalSim", BaseConfig.phoneMODEL)
            //.post(RequestBody.create(MediaType.parse("text/x-markdown; charset=utf-8"), params))
            .post(RequestBody.create(MediaType.parse("application/json; charset=utf-8"), params))
            .build();
//        return request;
    }
    ...
    public static abstract class ResultCallback<T> {
        ...
        public abstract void onResponse(Request request, T response, Headers headers);
    }
