...
public class CustomCurveChart extends View {
    ...
    public CustomCurveChart(Context context, String[] xLabel, String[] yLabel,
                            List<float[]> dataList, List<Integer> colorList, boolean showValue) {
        super(context);
        this.xLabel = xLabel;
        this.yLabel = yLabel;
    ...
    public void init() {
        ...
        paintAxes.setColor(ContextCompat.getColor(getContext(), R.color.color_999999));
        ...
        paintCoordinate.setColor(ContextCompat.getColor(getContext(), R.color.color_999999));
    ...
    private void drawCurve(Canvas canvas, Paint paint, float[] data, int color) {
        ...
        for (int i = 0; i <= (xLabel.length - 1); i++) {
            if (i == 0) {
                path.moveTo(xPoint, toY(data[0] - 1200));
            } else {
                path.lineTo(xPoint + i * xScale, toY(data[i] - 1200));
            }
    ...
    private void drawValue(Canvas canvas, Paint paint, float data[], int color) {
        ...
        for (int i = 1; i <= (xLabel.length - 1); i++) {
            ...
            if (toY(data[i - 1]) < toY(data[i])) {
                rect = new RectF(xPoint + i * xScale - 40, toY(data[i] - 1200) - 15,
                        xPoint + i * xScale + 40, toY(data[i] - 1200) + 5);
                canvas.drawRoundRect(rect, 5, 5, paint);
                canvas.drawText(String.valueOf(data[i]), xPoint + i * xScale, toY(data[i] - 1200), paintValue);
            } else if (toY(data[i - 1] - 1200) > toY(data[i] - 1200)) {
                rect = new RectF(xPoint + i * xScale - 40, toY(data[i] - 1200) - 5,
                        xPoint + i * xScale + 40, toY(data[i] - 1200) + 15);
                canvas.drawRoundRect(rect, 5, 5, paint);
                canvas.drawText(String.valueOf(data[i]), xPoint + i * xScale, toY(data[i] - 1200) + 10, paintValue);
            } else {
                rect = new RectF(xPoint + i * xScale - 40, toY(data[i] - 1200) - 10,
                        xPoint + i * xScale + 40, toY(data[i] - 1200) + 10);
                canvas.drawRoundRect(rect, 5, 5, paint);
                canvas.drawText(String.valueOf(data[i]), xPoint + i * xScale, toY(data[i] - 1200) + 5, paintValue);
            }
        }
    }
